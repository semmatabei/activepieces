...
const CANCELLABLE_STATUSES: FlowRunStatus[] = [
    FlowRunStatus.PAUSED,
    FlowRunStatus.QUEUED,
]

export const WEBHOOK_TIMEOUT_MS =
  system.getNumberOrThrow(AppSystemProp.WEBHOOK_TIMEOUT_SECONDS) * 1000
...
export const flowRunService = (log: FastifyBaseLogger) => ({
    ...
    async list(params: ListParams): Promise<SeekPage<FlowRun>> {
        ...
        if (!isNil(params.failedStepName)) {
            query = query.andWhere(
                'flow_run."failedStep"->>\'name\' = :failedStepName',
                {
    ...
    async retry({
        ...
        strategy,
        projectId,
    }: RetryParams): Promise<FlowRun> {
        const oldFlowRun = await flowRunService(log).getOnePopulatedOrThrow({
            id: flowRunId,
            projectId,
        })
        log.info(
            { flowRun: { id: flowRunId }, flow: { id: oldFlowRun.flowId }, strategy },
            'Flow run retry initiated',
        )

        const retentionDays = system.getNumberOrThrow(
            AppSystemProp.EXECUTION_DATA_RETENTION_DAYS,
        )
        if (
            isFlowRunStateTerminal({
                status: oldFlowRun.status,
      ...
      isOutsideRetentionWindow(oldFlowRun.created, retentionDays)
        ...
        switch (strategy) {
            case FlowRetryStrategy.FROM_FAILED_STEP: {
                ...
                const platformId = await projectService(log).getPlatformId(
                    updatedFlowRun.projectId,
                )
            ...
            case FlowRetryStrategy.ON_LATEST_VERSION: {
                const latestFlowVersion = await flowVersionService(
                    log,
                ).getLatestLockedVersionOrThrow(oldFlowRun.flowId)
                const triggerStep = oldFlowRun.steps?.[latestFlowVersion.trigger.name]
                const triggerFailed = triggerStep?.status === StepOutputStatus.FAILED
                const payload = await resolveStepOutput({
                    step: triggerStep,
                    flowRun: oldFlowRun,
                    log,
                })
                return this.start({
                    ...
                    payload,
                    platformId: await projectService(log).getPlatformId(
                        oldFlowRun.projectId,
                    ),
                    executionType: ExecutionType.BEGIN,
                    streamStepProgress: StreamStepProgress.NONE,
                    workerHandlerId: undefined,
    ...
    async cancel({
        ...
        const cancelParentFlowRuns = await Promise.allSettled(
            flowRuns.map((flowRun) => cancelSingleRun(log, flowRun, platformId)),
        )
        const childFlows = await getAllChildRuns(
            flowRuns.map((flowRun) => flowRun.id),
        )
        log.info(
            {
        ...
        const canceChildlPromises = await Promise.allSettled(
            childFlows.map((flowRun) => cancelSingleRun(log, flowRun, platformId)),
        )
    ...
    async bulkRetry(params: BulkRetryParams): Promise<FlowRunWithRetryError[]> {
        ...
        return results.map((result, i) => {
            ...
            const error =
        result.reason instanceof ActivepiecesError ? result.reason : undefined
            return {
                ...filteredFlowRuns[i],
                error: {
    ...
    async start({
        ...
        return createBindProjectEnqueueRun({
            flowId,
            payload,
            executeTrigger,
            executionType,
    ...
    async test({
        ...
        const testPassgradContext = resolveTestPassgradContext({
        ...
        return createBindProjectEnqueueRun({
            ...
            sampleData: !isNil(stepNameToTest)
                ? await sampleDataService(log).getSampleDataForFlow(
                    projectId,
                    flowVersion,
                    SampleDataFileType.OUTPUT,
                )
                : undefined,
            resolveAdmission: ({ flowRun }) =>
                testPassgradContext
                    ? passgradAdmissionService
                        .admit({
                            allowTesting: true,
                            flowId: flowVersion.flowId,
                            invocationId: flowRun.id,
                            logger: log,
                            projectId,
                            runEnvironment: RunEnvironment.TESTING,
                            sourceSubmissionId: testPassgradContext.sourceSubmissionId,
                            triggerKind: 'form_submission',
                        })
                        .then((result) =>
                            result.status === 'admitted' ? result.admission : undefined,
                        )
    ...
    async startManualTrigger({
        ...
        flowVersionId,
        triggeredBy,
    }: StartManualTriggerParams): Promise<FlowRun> {
        const flowVersion = await flowVersionService(log).getOneOrThrow(
            flowVersionId,
        )
        await flowService(log).getOneOrThrow({ id: flowVersion.flowId, projectId })
    ...
    async getOne(params: GetOneParams): Promise<FlowRun | null> {
        const flowRun = await queryBuilderForFlowRun(flowRunRepo())
            .where({
                id: params.id,
    ...
    async getStepsOrNull({
        flowRun,
    }: {
        flowRun: FlowRun
    }): Promise<Record<string, StepOutput> | null> {
        if (isNil(flowRun.logsFileId)) {
            return null
        }
        const stateFile = await readLogsFile(
            log,
            flowRun.logsFileId,
            flowRun.projectId,
        )
    ...
    async countByStatus(
        ...
        return results.map((r: { status: FlowRunStatus, count: string }) => ({
            status: r.status,
...
async function getAllChildRuns(parentRunIds: string[]): Promise<FlowRun[]> {
    ...
    const params = [parentRunIds, CANCELLABLE_STATUSES]
...
export async function createBindProjectEnqueueRun(
    params: CreateBindProjectEnqueueRunParams,
): Promise<FlowRun> {
    const flowRun = await queueOrCreateInstantly(
        {
            projectId: params.projectId,
    ...
    try {
        const admission =
      params.admission ?? (await params.resolveAdmission?.({ flowRun }))
        if (admission) {
        ...
        await addToQueue(
            {
                ...
                executionType: params.executionType,
                workerHandlerId: params.workerHandlerId,
                httpRequestId: params.httpRequestId,
                streamStepProgress: params.streamStepProgress,
                sampleData: params.sampleData,
                passgradContext: admission
                    ? toPassgradRunContext(admission)
                    : undefined,
            },
            params.log,
        )
        await flowRunSideEffects(params.log).onStart(flowRun)
        params.log.info(
            {
                flowRun: { id: flowRun.id },
                flow: { id: params.flowId },
        ...
        await markRunStartFailed(flowRun, params.log, error)
        throw error
    }
}
...
export async function addToQueue(
    ...
    if (!isNil(params.payload) && isNil(params.workerHandlerId)) {
        jobPayload = await payloadOffloader.offloadPayload(
            log,
            params.payload,
            params.flowRun.projectId,
            params.platformId,
        )
    }
    else if (!isNil(params.payload)) {
        jobPayload = await payloadOffloader.maybeOffloadPayload(
            log,
            params.payload,
            params.flowRun.projectId,
            params.platformId,
    ...
    const passgradContext =
    params.passgradContext ??
    (await passgradAdmissionService.recover({
    ...
    params.executionType === ExecutionType.RESUME
        ? {
            ...commonJobData,
            executionType: ExecutionType.RESUME,
...
export async function findFlowRunOrThrow(
    flowRunId: FlowRunId,
): Promise<FlowRun> {
    const flowRun = await queryBuilderForFlowRun(flowRunRepo())
        .where({ id: flowRunId })
...
function queryBuilderForFlowRun(
    repo: Repository<FlowRun>,
): SelectQueryBuilder<FlowRun> {
    return repo
        .createQueryBuilder('flow_run')
...
async function resolveStepOutput({
    ...
    flowRun,
    log,
}: ResolveStepOutputParams): Promise<unknown> {
    if (isNil(step)) {
        return undefined
    }
...
async function queueOrCreateInstantly(
    params: CreateParams,
...
async function markRunStartFailed(
    flowRun: FlowRun,
    log: FastifyBaseLogger,
    error: unknown,
): Promise<void> {
    ...
    log.error(
        { flowRun: { id: flowRun.id }, error },
        'Flow run failed before enqueue',
    )
}

export function isOutsideRetentionWindow(
    createdTime: string,
...
type AddToQueueParamsCommon = {
    ...
    passgradContext?: PassgradRunContext
}
...
type CreateBindProjectEnqueueRunParams = Omit<StartParams, 'admission'> & {
    log: FastifyBaseLogger
    admission?: WorkflowAdmission
    resolveAdmission?: (params: {
        flowRun: FlowRun
    }) => Promise<WorkflowAdmission | undefined>
}

function toPassgradRunContext(
    admission: WorkflowAdmission,
): PassgradRunContext {
    return {
        sourceSubmissionId: admission.sourceSubmissionId,
        triggerKind: admission.triggerKind,
        workflowId: admission.workflowId,
    }
}

function resolveTestPassgradContext({
    flowId,
    triggerPayload,
}: {
    flowId: FlowId
    triggerPayload: unknown
}): PassgradRunContext | undefined {
    const payload = Array.isArray(triggerPayload)
        ? triggerPayload[0]
        : triggerPayload
    if (!isRecord(payload)) return undefined
    const submission = isRecord(payload.submission)
        ? payload.submission
        : payload
    if (
        typeof submission.id !== 'string' ||
    typeof submission.formId !== 'string' ||
    typeof submission.formVersionId !== 'string' ||
