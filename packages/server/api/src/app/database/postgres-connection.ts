...
import { AddPassgradLifecycleOutbox1796300000000 } from './migration/postgres/1796300000000-AddPassgradLifecycleOutbox'
...
export const getMigrations = (): (new () => Migration)[] => {
    const migrations = [
        ...
        // Community Only Migrations, before unifying all migrations.
        AddPlatformToPostgres1709052740378,
        SetNotNullOnPlatform1709505632771,

        // New Migration After Unifying cloud and enterprise
        ...
        AddPassgradLifecycleOutbox1796300000000,
    ]
    return migrations
}
...
export const createPostgresDataSource = (): DataSource => {
    ...
    if (!isNil(url)) {
        return new DataSource({
            ...
            ...spreadIfDefined(
                'poolSize',
                system.get(AppSystemProp.POSTGRES_POOL_SIZE),
            ),
            ...migrationConfig,
            ...commonProperties,
        })
    ...
    const idleTimeoutMillis = system.getNumberOrThrow(
        AppSystemProp.POSTGRES_IDLE_TIMEOUT_MS,
    )
