...
function capabilitySecretValidator(value: string) {
    return Buffer.byteLength(value ?? '') >= 32 ? true : 'Value must contain at least 32 bytes'
}
...
const systemPropValidators: {
    ...
    [AppSystemProp.LOG_LEVEL]: enumValidator([
        'error',
        'warn',
        'info',
        'debug',
    ...
    [AppSystemProp.PASSGRAD_ADMISSION_URL]: urlValidator,
    [AppSystemProp.PASSGRAD_ADMISSION_SECRET]: stringValidator,
    [AppSystemProp.PASSGRAD_ALLOWED_ORIGINS]: stringValidator,
    [AppSystemProp.PASSGRAD_PLATFORM_ID]: stringValidator,
    [AppSystemProp.PASSGRAD_PROVISIONING_SECRET]: stringValidator,
    [AppSystemProp.PASSGRAD_API_URL]: urlValidator,
    [AppSystemProp.PASSGRAD_ENGINE_CAPABILITY_SECRET]: capabilitySecretValidator,
    ...
    [AppSystemProp.FILE_STORAGE_LOCATION]: enumValidator(
        Object.values(FileLocation),
    ),
    ...
    [AppSystemProp.SCIM_DEFAULT_PROJECT_ROLE]: enumValidator(
        Object.values(DefaultProjectRole),
    ),
...
export const validateEnvPropsOnStartup = async (
    ...
    if (
        environment !== ApEnvironment.TESTING &&
    fileStorageLocation === FileLocation.S3
    ) {
        try {
            ...
            throw new Error(
                JSON.stringify({
                    error: inspect(error),
                    message:
            'S3 validation failed. Check your configuration and credentials.',
                    docUrl:
            'https://www.activepieces.com/docs/install/configuration/overview#configure-s3-optional',
                }),
            )
        }
    }
    ...
    if (!isNil(codeSandboxType)) {
        throw new Error(
            JSON.stringify({
                message:
          'AP_CODE_SANDBOX_TYPE is deprecated, please use AP_EXECUTION_MODE instead',
                docUrl:
    ...
    const isValidHexKey =
    encryptionKey && /^[A-Za-z0-9]{32}$/.test(encryptionKey)
    if (!isValidHexKey) {
        throw new Error(
            JSON.stringify({
                message:
    ...
    if (isApp) {
        const rentionPeriod = system.getNumberOrThrow(
            AppSystemProp.EXECUTION_DATA_RETENTION_DAYS,
        )
        const maximumPausedFlowTimeout = system.getNumberOrThrow(
        ...
        if (maximumPausedFlowTimeout > rentionPeriod) {
            throw new Error(
                JSON.stringify({
                    message:
            'AP_PAUSED_FLOW_TIMEOUT_DAYS can not exceed AP_EXECUTION_DATA_RETENTION_DAYS',
                }),
            )
        }
    }
    ...
    if (isNil(jwtSecret)) {
        throw new Error(
            JSON.stringify({
                message:
          'AP_JWT_SECRET is undefined, please define it in the environment variables',
                docUrl:
          'https://www.activepieces.com/docs/install/configuration/environment-variables',
            }),
        )
