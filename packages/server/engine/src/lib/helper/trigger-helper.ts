...
import {
    EngineConstants,
    ResolvedExecuteTriggerOperation,
} from '../handler/context/engine-constants'
...
export const triggerHelper = {
    async executeOnStart(
        ...
        const { pieceName, pieceVersion, triggerName, input, propertySettings } = (
            trigger as PieceTrigger
        ).settings

        if (isNil(triggerName)) {
            throw new EngineGenericError(
                'TriggerNameNotSetError',
                'Trigger name is not set',
            )
        }

        const { pieceTrigger, processedInput, piece } =
      await prepareTriggerExecution({
          pieceName,
          pieceVersion,
          triggerName,
          input,
        ...
        const context = {
            ...
            passgrad: createPassgradCapability({
                apiUrl: constants.internalApiUrl,
                credential: constants.passgradCapability,
            }),
        }
    ...
    async executeTrigger({
        ...
        const { pieceName, pieceVersion, triggerName, input, propertySettings } = (
            params.flowVersion.trigger as PieceTrigger
        ).settings
        ...
        const { piece, pieceTrigger, processedInput } =
      await prepareTriggerExecution({
          pieceName,
          pieceVersion,
          triggerName,
        ...
        switch (params.hookType) {
            ...
            case TriggerHookType.ON_ENABLE: {
                ...
                return {
                    ...
                    scheduleOptions:
            pieceTrigger.type === TriggerStrategy.POLLING
                ? scheduleOptions
                : undefined,
                }
            }
            ...
            case TriggerHookType.HANDSHAKE: {
                const { data: handshakeResponse, error: handshakeResponseError } =
          await utils.tryCatchAndThrowOnEngineError(() =>
              pieceTrigger.onHandshake(context),
          )
            ...
            case TriggerHookType.TEST: {
                const { data: testResponse, error: testResponseError } =
          await utils.tryCatchAndThrowOnEngineError(() =>
              pieceTrigger.test({
                  ...context,
            ...
            await utils.tryCatchAndThrowOnEngineError(async () => {
                if (!params.appWebhookUrl) {
                    throw new EngineGenericError(
                        'AppWebhookUrlNotAvailableError',
                        `App webhook url is not available for piece name ${pieceName}`,
                    )
                }
                if (!params.webhookSecret) {
                    throw new EngineGenericError(
                        'WebhookSecretNotAvailableError',
                        `Webhook secret is not available for piece name ${pieceName}`,
                    )
                }
                ...
                const { data: triggerRunResult, error: triggerRunError } =
          await utils.tryCatchAndThrowOnEngineError(async () => {
...
async function prepareTriggerExecution({
    pieceName,
    pieceVersion,
    triggerName,
    input,
