...
export const pieceHelper = {
  ...
  > {
    ...
    const { data: executePropsResult, error: executePropsError } =
          ...
          switch (property.type) {
            ...
            case PropertyType.MULTI_SELECT_DROPDOWN: {
              const multiSelectProperty =
                property as MultiSelectDropdownProperty<unknown, boolean>;
              const options = await multiSelectProperty.options(
                resolvedInput,
                ctx
              );
              return {
            ...
            case PropertyType.DROPDOWN: {
              const dropdownProperty = property as DropdownProperty<
                ...
                boolean
              >;
              const options = await dropdownProperty.options(
                resolvedInput,
                ctx
              );
              return {
  ...
  async executeValidateAuth({
    ...
    const server = {
      apiUrl: params.internalApiUrl.endsWith('/')
        ? params.internalApiUrl
        : params.internalApiUrl + '/',
  ...
  async extractPieceMetadata({
    ...
    const pieceAlias = pieceLoader.getPackageAlias({
      pieceName,
      pieceVersion,
      devPieces,
    });
    const pieceIndexPath = await pieceLoader.getPiecePath({
      packageName: pieceAlias,
      devPieces,
    });
...
type ExecutePropsParams = Omit<ExecutePropsOptions, 'piece'> & {
  pieceName: string;
  pieceVersion: string;
};
