...
const executeAction: ActionHandler<PieceAction> = async ({
    ...
    await utils.tryCatchAndThrowOnEngineError(async () => {
        ...
        const { resolvedInput, censoredInput } = await constants
            .getPropsResolver(piece.getContextInfo?.().version)
        ...
        await propsProcessor.applyProcessorsAndValidators(
            ...
            action.settings.propertySettings,
        )
        ...
        if (!isPaused) {
            await flowRunProgressReporter.sendUpdate({
                engineConstants: constants,
                flowExecutorContext: await executionState.upsertStep(
                    action.name,
                    stepOutput,
                ),
        ...
        const context: ActionContext<PieceAuthProperty, InputPropertyMap> = {
            ...
            agent: {
                tools: async (params: ConstructToolParams): Promise<ToolSet> =>
                    agentTools.tools({
                        engineConstants: constants,
                        tools: params.tools,
                        model: params.model,
        ...
        testSingleStepMode && !isNil(pieceAction.test)
            ? pieceAction.test
            : pieceAction.run
        const output = await runMethodToExecute(backwardCompatibleContext)
        const newExecutionContext = executionState.addTags(
            params.hookResponse.tags,
        )

        const webhookResponse = getResponse(params.hookResponse)
        const isSamePiece =
        constants.triggerPieceName === action.settings.pieceName
        if (
            !isNil(webhookResponse) &&
        !isNil(constants.workerHandlerId) &&
        !isNil(constants.httpRequestId) &&
        isSamePiece
        ) {
        ...
        if (params.hookResponse.type === 'stopped') {
            if (isNil(params.hookResponse.response)) {
                throw new EngineGenericError(
                    'StopResponseNotSetError',
                    'Stop response is not set',
                )
            }
            ...
            return (await newExecutionContext.upsertStep(action.name, succeeded))
                .incrementStepsExecuted()
                .setVerdict({
                    status: FlowRunStatus.SUCCEEDED,
                    stopResponse: (params.hookResponse.response as StopHookParams)
                        .response,
                })
        }
    ...
    if (executionStateError) {
        ...
        return (
            await executionState.upsertStep(action.name, failedStepOutput)
        ).setVerdict({
...
function createWaitForWaitpointHook({
    hookParams,
}: {
    hookParams: { hookResponse: HookResponse }
}): WaitForWaitpointHook {
